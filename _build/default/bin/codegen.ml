...
let state:PrgmSt.t ref = ref PrgmSt.empty
...
let rec codegen_statement (s:statement) (* (state:PrgmSt.t) *) (return_type:typ) = 
  match s with
  | Scope(sl) -> state:= PrgmSt.push_stack !state;Printf.printf "new scope";codegen_scope sl (* state *) return_type
  | ExpressionStatement(e) -> (codegen_expr e(*  state *)) |> ignore (* might have a secondary thing for exprs with side effects *)
  ...
  | VarDefinition(_, t, name, value) -> 
    ...
    let alloca = 
      ...
      state := (PrgmSt.add_var !state name (alloca, t));
      let v = 
        (match value with
...
and codegen_scope (scope:statement list) (return_type:typ) :unit = 
  match scope with
  | x :: xs -> codegen_statement x (* state *) return_type; codegen_scope xs (* state *) return_type
  | [] -> state := (PrgmSt.pop_stack !state);

and codegen_function return_type name (params:(typ * string) list) body state =
  (* PrgmSt.push_stack state |> ignore; *) (* not sure if nec (probably not because yeah) *)
